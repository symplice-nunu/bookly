  ...
  return (
    <div class="flex justify-center absolute w-[100%] z-[1]">
      <div className="flex items-center justify-between text-white px-[110px] text-[12px] header">
        <Link to="/">
          <div className="py-4">
            <img className="h-[40px]" src={logo_bookly} alt="" />
          </div>
        </Link>
          ...
          {navData.map((item) => {
            return (
              <Link to={item.path}>
                <div
                  className={`${
                    active === item.path ? "bg-teal-500" : null
                  } flex gap-1 hover:bg-teal-500 px-4 py-1 rounded`}
                >
        ...
        <div className="flex space-x-4 py-8">
          <div className="flex space-x-1 pt-1">
            <div>
              <img className="h-[15px]" src={usaLogo} alt="" />
            </div>
            <div>
              <RxTriangleDown />
            </div>
